...
public:
	...
	UnitGroup* CreateGroupFromEntities(std::list<DynamicEntity*> units);
...
private:
	...
	std::list<UnitGroup*> unitGroups; // contains all the existing groups
...
struct UnitGroup
{
	...
	UnitGroup(std::list<DynamicEntity*> units);
	...
	std::list<SingleUnit*> units; // contains all the units of a given group
...
struct SingleUnit
{
	...
	std::vector<iPoint> path; // path to the unit's goal
