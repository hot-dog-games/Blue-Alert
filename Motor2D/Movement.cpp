...
bool Movement::CleanUp()
{
	...
	std::list<UnitGroup*>::const_iterator it = unitGroups.begin();
...
UnitGroup* Movement::CreateGroupFromEntities(std::list<DynamicEntity*> units)
{
	std::list<DynamicEntity*>::const_iterator it = units.begin();
...
UnitGroup* Movement::CreateGroupFromUnit(DynamicEntity* unit)
{
	// If a unit from the list belongs to an existing group, delete the unit from the group
	UnitGroup* group = nullptr;
...
UnitGroup* Movement::GetLastGroup() const
{
...
UnitGroup* Movement::GetGroupByUnits(std::list<DynamicEntity*> units) const
{
	std::list<DynamicEntity*>::const_iterator it;
	std::list<UnitGroup*>::const_iterator groups;
	std::list<SingleUnit*>::const_iterator singleUnits;
	uint size = 0;
...
MovementState Movement::MoveUnit(DynamicEntity* unit, float dt)
{
	...
	case MovementState_FollowPath:
		...
		movePos = { (float)nextPos.x - u->unit->position.x, (float)nextPos.y - u->unit->position.y };
		...
		endPos = { u->unit->position.x + movePos.x,u->unit->position.y + movePos.y };

		if (u->IsTileReached(nextPos, endPos)) {

			u->unit->position.x = nextPos.x;
			u->unit->position.y = nextPos.y;
		...
		u->unit->position.x += movePos.x;
		u->unit->position.y += movePos.y;
...
void Movement::CheckForFutureCollision(SingleUnit* singleUnit) const
{
			...
			if ((*units) != singleUnit) {
				...
				if (singleUnit->coll != CollisionType_TowardsCell && (*units)->coll != CollisionType_TowardsCell) {
					...
					// TODO 5c: Check if two units would reach the same tile
						...
						EntiyDirection dirA = singleUnit->unit->GetUnitDirection();
						EntiyDirection dirB = (*units)->unit->GetUnitDirection();

						singleUnit->waitUnit = *units;
						singleUnit->waitTile = singleUnit->nextTile;

						// The agent that is already inside the tile has the priority. If none of them are, choose one randomly
						SDL_Rect posA = { (int)singleUnit->unit->position.x, (int)singleUnit->unit->position.y, App->map->data.tile_width, App->map->data.tile_height };
						SDL_Rect posB = { (int)(*units)->unit->position.x, (int)(*units)->unit->position.y,  App->map->data.tile_width, App->map->data.tile_height };
...
bool Movement::IsValidTile(SingleUnit* singleUnit, iPoint tile, bool currTile, bool nextTile, bool goalTile) const
{
...
iPoint Movement::FindNewValidTile(SingleUnit* singleUnit, bool checkOnlyFront) const
{
...
iPoint Movement::FindNewValidGoal(SingleUnit* singleUnit, iPoint goal, bool checkEverything)
{
...
bool Movement::ChangeNextTile(SingleUnit* singleUnit)
{
	bool ret = false;
...
bool Movement::IsOppositeDirection(SingleUnit* singleUnitA, SingleUnit* singleUnitB) const
{
...
UnitGroup::UnitGroup(std::list<DynamicEntity*> units)
{
	std::list<DynamicEntity*>::const_iterator it = units.begin();
...
UnitGroup::~UnitGroup()
{
	std::list<SingleUnit*>::const_iterator it = units.begin();
...
bool UnitGroup::RemoveUnit(SingleUnit* singleUnit)
{
	...
	std::list<SingleUnit*>::const_iterator it = find(units.begin(), units.end(), singleUnit);
...
{
	currTile = goal = newGoal = App->map->WorldToMap(this->unit->position.x, this->unit->position.y);
...
bool SingleUnit::CreatePath(iPoint startPos)
{
	...
	// Find a path
	if (App->pathfinding->CreatePath(startPos, goal) == -1)
		ret = false;
